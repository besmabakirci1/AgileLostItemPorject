...
export default async function KayipIlanDetayPage({
  params,
}: {
      ...
      <div className="card overflow-hidden p-6 sm:p-8">
        <div className="flex gap-5">
          {item.imageUrl ? (
            ...
            <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-lost-light text-4xl">
              🔍
            </span>
          )}
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-5 flex flex-wrap gap-2 text-sm text-neutral-500">
              ...
              <span>Kayıp tarihi: {new Date(item.lostAt).toLocaleDateString('tr-TR')}</span>
            </div>
            {item.user && (
