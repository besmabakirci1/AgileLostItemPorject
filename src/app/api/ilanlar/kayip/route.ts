...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { title, description, category, place, lostAt, imageUrl } = body;
    if (!title || !description || !category || !place || !lostAt) {
      return NextResponse.json(
        { error: 'Başlık, açıklama, kategori, yer ve kayıp tarihi gerekli.' },
        { status: 400 }
      );
    }
